...
(function ($) {
    ...
    $.widget("jHtree.jHTree", {
        ...
        _constructTree: function (jsonStructureObject) {
            ...
            this._walkerCursor(jsonStructureObject, 'tremainul','tree_0');
            this._prepareNodes();
            this._treeDarg();
            this._treeDrop();
        ...
        _walkerCursor: function (jsonObjs, parentLiNode,index) {
            for (var i = 0; i < jsonObjs.length; i++) {
                var node = jsonObjs[i];
				treeData[node['id']] = index;
				
                this._createNode(node, parentLiNode);

                if (node.children !== null && typeof node.children === "object") {
                    this._walkerCursor(node.children, node.id, node['id']);
                }
            }
        },
        ...
        _treeDrop: function () {
            ...
            $("li", ".tree").droppable({
                ...
                drop: function (event, ui) {
					...
					// Custom by Yusuf Eko N.
					// Change data Tree
					treeData[draggableId] = droppableId;
